...
void enter_wait(void)
{
     ...
     */
    /*
    * 清除SLEEPDEEP位，确保可以进去WAIT模式而不是deep sleep
    */
	SCB_SCR &= ~SCB_SCR_SLEEPDEEP_MASK;
...
int enter_vlpr(void)
{
  ...
  if ((SMC_PMSTAT & SMC_PMSTAT_PMSTAT_MASK)== 4){  /* 判断是否已经进入了VLPR模式 */
         return_value = 0x14;                      /* 如果是，返回错误代码，标明已经进入 */
         }
  /* The PMPROT register may have already been written by init code
  ...
  SMC_PMPROT = SMC_PMPROT_AVLP_MASK;  /* 该寄存器每次复位之后只能写一次，如果之前有写过，则这一次不起作用 */
                                      /* 允许进入VLPR、VLPW和VLPS模式 */
  ...
  SMC_PMCTRL  |= SMC_PMCTRL_RUNM(0x2); /* 进入VLPR模式 */
  ...
  for (i = 0 ; i < 10000 ; i++)
    {     /* check that the value of REGONS bit is not 0
      ...
      /* 检查内部电压调节器的状态，如果是0，说明进入停止模式，或正在转换模式 */
  ...
  if ((SMC_PMSTAT & SMC_PMSTAT_PMSTAT_MASK) == 4)  /* 判断是否已经进入了VLPR模式 */
    {
      return_value = SMC_PMSTAT;                   /* 如果是，返回状态寄存器的值 */
    }
  return (return_value);
}
/********************************************************************/
...
void exit_vlpr(void)
{
    ...
    if  ((SMC_PMSTAT & SMC_PMSTAT_PMSTAT_MASK)== 4) {                  /* 判断是否运行在VLPR模式         */
      
       /* Clear RUNM */
       SMC_PMCTRL &= ~SMC_PMCTRL_RUNM_MASK;                            /* 切换到RUN模式下运行            */
       ...
       for (i=0;i<0xff;i++)
       {
           if ((SMC_PMSTAT & SMC_PMSTAT_PMSTAT_MASK)== SMC_PMSTAT_PMSTAT_MASK)
             if ((PMC_REGSC & PMC_REGSC_REGONS_MASK) == PMC_REGSC_REGONS_MASK) /* 判断是否已经切换到RUN模式 */
    	        break;
       }           
    }  //if in VLPR mode
...
void enter_vlps(void)
{
  ...
  SMC_PMPROT = SMC_PMPROT_AVLP_MASK;           //仅仅允许AVPL 低功耗模式
